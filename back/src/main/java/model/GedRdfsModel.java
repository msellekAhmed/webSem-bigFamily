...
public class GedRdfsModel {


	private static final String GED_DATA_FILE = "data/ged.rdf";
	...
	public static Resource createMember(Person member) {
		String memberURI = "";
		if(member.getFirstName() != null && !member.getFirstName().equals("") && member != null){
			memberURI = "http://familytree/person/" + member.getFirstName();
		}else{
			memberURI = "http://familytree/person/" + member.getFirstName();
		}
...
//			}
	...
	public static void insertGedData(){
		FileWriter fw = null;
		try {
			fw = new FileWriter(GED_DATA_FILE, false);
		} catch (IOException e) {
			e.printStackTrace();
		}
